...
There is a layered lookup system. Use it rather than blindly grepping all 37 chapter files:

| File | Purpose |
|---|---|
| **`README.md`** | Human-facing entry point: orientation, links. |
| **`index.md`** (repo root) | Master cross-book index: chapter map for all 4 books + merged A–Z concept index. **Always start here when the user asks where a topic is discussed.** |
| **`book-<N>-*/index.md`** | Per-book index: chapter title + 2–4 sentence summary + key concepts list, then a book-level A–Z concept index. |
...
3. If the question needs a quote or fine detail → read the chapter file itself, citing using the **B`<n>`/`<chapter>`#p`<paragraph>`** form.
4. For verbatim phrase search → `grep -ril "<phrase>" book-*/` against the source files directly. **Do not grep the indexes — they paraphrase.**
...
- The master `index.md` uses the compact form **B`<n>`: <prefix>** (e.g. *B2: 03, 05*).

## Notation in the master index

- Chapter refs use the leading two-digit prefix only: e.g. **B2: 03, 05** means *Book 2, files starting with `03-…` and `05-…`*.
...
title: "<full title>"
type: front-matter | briefing | commentary | final-words | back-matter
key_concepts:
  - "..."
  - "..."
---
...
- Keep the **B`<n>`: <prefix>** notation in the master index — don't expand to full filenames.
